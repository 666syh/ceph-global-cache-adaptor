...
#define HDD_MIN_ALLOC_SIZE_NAME "blustore_min_alloc_size_hdd"
#define SSD_MIN_ALLOC_SIZE_NAME "blustore_min_alloc_size_ssd"
...
void RadosBindMsgrWorker(std::vector<uint32_t> coreId, pid_t pid)
{
    ...
    if (r != 0) {
	 ProxyDbgLogErr("readdir (%s) failed: %d", path, r);
	 return;
    }
...
int RadosClientInit(rados_client_t *client,const std::string &cephConf)
{
	...
	if (ret != 0) {
        ...
        ProxyDbgLogInfo("set conf<%s, %s> success.", ProxyGetMonTimeOutOption(), str.c_str());
	...
	if (ret != 0) {
        ...
        ProxyDbgLogInfo("set conf<%s, %s> success.", ProxyGetOsdTimeOutOption(), str.c_str());
	...
	if (ret != 0) {
        ...
        ProxyDbgLogInfo("set conf<%s, %s> success.", LOG_FILE_NAME, ProxyGetLogPath());
...
int RadosCreateIoCtx(rados_client_t client, const std::string &poolname,rados_ioctx_t *ctx)
{
	...
	if (ret < 0) {
		ProxyDbgLogErr("create ioctx failed.");
		return ret;
	}
...
int RadosCreateIoCtx2(rados_client_t client, const int64_t poolId,rados_ioctx_t *ctx)
{
	...
	if ( ret < 0 ) {	
		ProxyDbgLogErr("create ioctx by poolId failed.");
		return ret;
	}
...
void RadosReadOpGetXattrs(rados_op_t op, proxy_xattrs_iter_t *iter, int *prval)
{
	...
	if (xIter == nullptr) {
		ProxyDbgLogErr("Allocate xIter Failed.");
		return nullptr;
	}
...
void WriteCallback(librados::completion_t c, void *arg)
{
